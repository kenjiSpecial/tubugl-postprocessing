...
const fragmentSrc = require('./postprocess.glsl');
...
export default class App {
	...
	loop() {
		...
		if (this.isPost) this._postProcess.bind();
		...
		if (this.isPost)
			this._postProcess
				.unbind()
				.render([
					{ name: 'uWindow', type: 'vec2', value: [this._width, this._height] },
					{ name: 'uRandom', type: 'float', value: randomFloat(0.9, 1) }
				]);
	}
	...
	_makeSpheres() {
		...
		for (let ii = 0; ii < 10; ii++) {
			let side = mathUtils.randomFloat(30, 100);
			let sphere = new Sphere(this.gl, { isWire: true }, side, 5, 5);
